...
typedef PACKED_STRUCT xbee_dispatch_table_entry {
...
enum xbee_dev_flags
{
	...
	XBEE_DEV_FLAG_USE_FLOWCONTROL	= 0x8000,	///< Check CTS before sending
};
...
/**
	...
	-	Uses function pointers for setting the reset pin and reading the awake
		...
		about the reset.
*/
typedef PACKED_STRUCT xbee_dev_t
{
	/// Generic WPAN device required by the \ref zigbee layers of the API.
	wpan_dev_t				wpan_dev;

	/// Pointer to a Platform-specific structure required by xbee_serial.c
...
/**
	...
	The application needs to define this table, and it must end with
...
int xbee_dev_init( xbee_dev_t *xbee, xbee_serial_t *usart, xbee_is_awake_fn is_awake, xbee_reset_fn reset);
