...
int xbee_cmd_tick( void)
{
		...
		i; ++request, --i)
	{
		if (request->device && XBEE_CHECK_TIMEOUT_SEC(request->timeout))
		{
			...
			#ifdef XBEE_ATCMD_VERBOSE
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: request 0x%04x timed out\n"), __FUNCTION__,
					expired.handle);
			#endif
			...
			if (request->callback)
			{
				...
				#ifdef XBEE_ATCMD_VERBOSE
					xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: dispatch to handler @%p w/context %" PRIpFAR "\n"),
			...
			{
				#ifdef XBEE_ATCMD_VERBOSE
					xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: releasing expired request 0x%04x\n"),
									...
									)
{
	...
	int16_t			request;
	uint32_t			param;
   const xbee_atcmd_reg_t FAR *reg = (xbee_atcmd_reg_t FAR *)(clc->list) + clc->index;

#ifdef XBEE_ATCMD_VERBOSE
   xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: next command AT%c%c\n"), __FUNCTION__,
   ...
   else
   {
      ...
      {
#ifdef XBEE_ATCMD_VERBOSE
   		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: can't create request (%d)\n"), __FUNCTION__,
         	request);
#endif
      	return request;
      }
...
int _xbee_cmd_list_callback( const xbee_cmd_response_t FAR *response)
{
   ...
   xbee_command_list_context_t FAR *clc = response->context;
	...
	{
#ifdef XBEE_ATCMD_VERBOSE
		 xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: timed out\n"), __FUNCTION__);
#endif
...
#ifdef XBEE_ATCMD_VERBOSE
		 	xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: found final callback\n"), __FUNCTION__);
#endif
   ...
   {
#ifdef XBEE_ATCMD_VERBOSE
      xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: matched command result AT%c%c\n"), __FUNCTION__,
            reg->command.str[0],reg->command.str[1]);
#endif
	   ...
	   if (reg->callback)
	   {
         ...
         // correspond to any list entry.
#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: final callback (error proc)\n"), __FUNCTION__);
#endif
	      reg->callback(response, NULL, clc->base);
			...
			{
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: Unexpected response AT%.*" PRIsFAR \
					" = 0x%0*" PRIX32 "\n"),
			...
			{
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: Unexpected %d-byte response to AT%.*" PRIsFAR "\n"),
...
#ifdef XBEE_ATCMD_VERBOSE
   xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: done\n"), __FUNCTION__);
#endif
   ...
   if (reg->callback)
   {
      // A final callback was specified (via XBEE_ATCMD_REG_END_CB)
#ifdef XBEE_ATCMD_VERBOSE
      xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: final callback\n"), __FUNCTION__);
#endif
      reg->callback(response, reg, clc->base);
   }
			...
			)
{
	...
	{
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: EO error 0x%02X\n"), __FUNCTION__,
			...
			)
{
	...
	{
		#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: AI=%" PRIu32 "\n"), __FUNCTION__,
				response->value);
		#endif
		if (response->value == 0)
		{
	...
	{
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: AI error 0x%02X\n"), __FUNCTION__,
				response->flags & XBEE_CMD_RESP_MASK_STATUS);
	}
#endif
}
			...
			)
{
   ...
   else if (response->flags & XBEE_CMD_RESP_FLAG_TIMEOUT)
   {
		xbee->flags |= XBEE_DEV_FLAG_QUERY_ERROR;
	#ifdef XBEE_ATCMD_VERBOSE
   	xSerialxPrintf_P( &xSerial1Port, PSTR("%s: AT%c%c timed out!\n"), __FUNCTION__,
   ...
   #ifdef XBEE_ATCMD_VERBOSE
      xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: HV=0x%x  VR=0x%" PRIx32 "  IEEE=%" PRIsFAR \
         "  net=0x%04" PRIx16 "\n"), __FUNCTION__,
         xbee->hardware_version, xbee->firmware_version,
         addr64_t_format( buffer, &xbee->wpan_dev.address.ieee),
...
int xbee_cmd_query_device( xbee_dev_t *xbee, uint_fast8_t refresh)
{
	...
	{
		#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: aborting; query already in progress\n"), __FUNCTION__);
		#endif
...
_xbee_atcmd_debug
int xbee_cmd_set_target( int16_t handle, const addr64_t FAR *ieee,
	...
	uint8_t length)
{
	...
	{
		#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: ERROR -EMSGSIZE (length %d > %d)\n"), __FUNCTION__,
	...
	{
		#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: ERROR -EINVAL (handle=0x%04x (%s))\n"),
...
int xbee_cmd_send( int16_t handle)
{
	...
	#ifdef XBEE_ATCMD_VERBOSE
	xSerialxPrint_P( &xSerial1Port, PSTR( "atcmd header is:"));
		hex_dump( &header, header_size, HEX_DUMP_FLAG_TAB);
		if (request->param_length)
		{
			xSerialxPrint_P( &xSerial1Port, PSTR( "atcmd param is:"));
	...
	uint16_t length, void FAR *context)
{
	...
	if (index == XBEE_CMD_REQUEST_TABLESIZE)
	{
		// didn't find request in table
		#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: couldn't match %.*s response to request table\n"),
			...
			if (value_length)
			{
				...
				*p = '\0';
				...
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s\n"), buffer);
			}
		#endif
	...
	#ifdef XBEE_ATCMD_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: response matched request %d\n"), __FUNCTION__, index);
	#endif
	...
	{
		#ifdef XBEE_ATCMD_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: no callback registered for %d\n"), __FUNCTION__, index);
		#endif
	}
	else
	{
	...
	const void FAR *payload, uint16_t length, void FAR *context)
{
		...
		status == XBEE_MODEM_STATUS_COORD_START)
	{
		// need to kick off discovery of my address
			...
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: looking up my network address (flags=0x%04X)\n"),
				__FUNCTION__, xbee->flags);
			error = xbee_cmd_query_device( xbee, 1);
			if (error)
			{
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: error %d querying device (flags=0x%04X)\n"),
	...
	uint32_t value)
{
	...
	#ifdef XBEE_ATCMD_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: sending AT%.2s=%" PRIu32 " (0x%" PRIx32 "), frame ID 0\n"),
	...
	const void FAR *data, uint8_t length)
{
	...
	#ifdef XBEE_ATCMD_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: sending AT%.2s, frame ID 0x%02x\n"),
