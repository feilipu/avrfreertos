...
void set_PWM_hardware( uint16_t servoA, uint16_t servoB)     // PWM pulse width in uS
{
    ...
    pwmOCRa = (uint8_t)( (float)servoA * configCPU_CLOCK_HZ  / pwmCLOCK_PRESCALER / 1000000 ); // This calculation is just wrong,
    pwmOCRb = (uint8_t)( (float)servoB * configCPU_CLOCK_HZ  / pwmCLOCK_PRESCALER / 1000000 ); // but I have no interest to fix it.

#elif defined( portUSE_TIMER1_PWM )

    // pwmOCRx is set in uS, so convert from uS to # of steps to pwmICR1
    pwmOCRa = (uint16_t)( (float)servoA * (configCPU_CLOCK_HZ/(pwmCLOCK_PRESCALER*pwmBASE_HZ) - 1) * pwmBASE_HZ / 1000000.0 );
    pwmOCRb = (uint16_t)( (float)servoB * (configCPU_CLOCK_HZ/(pwmCLOCK_PRESCALER*pwmBASE_HZ) - 1) * pwmBASE_HZ / 1000000.0 );
