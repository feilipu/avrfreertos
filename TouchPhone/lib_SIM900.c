...
/* Optionally, declare a reference to the handle for the serial port, USART0. */
extern xComPortHandle xSerialPort;
/* Optionally, declare a reference to the handle for the other serial port, USART1. */
...
uint8_t SIM900MakeCall(SIM900StateHandlePtr SIM900_State_Ptr, const char* tlfNumber){
	...
	uint8_t answer = waitForAnswer1(SIM900_State_Ptr, OK_RESPONSE, SIM900_MAX_TIME_OUT / portTICK_PERIOD_MS);
	return answer != 1 ? 0 : 1;
}
...
uint8_t SIM900Redial(SIM900StateHandlePtr SIM900_State_Ptr){
        ...
        uint8_t answer=sendCommand1( SIM900_State_Ptr, AT_REDIAL, OK_RESPONSE);
    	return answer != 1 ? 0 : 1;
}
...
uint8_t SIM900Answer(SIM900StateHandlePtr SIM900_State_Ptr){
        ...
        uint8_t answer=sendCommand1( SIM900_State_Ptr, AT_ANSWER, OK_RESPONSE);
    	return answer != 1 ? 0 : 1;
}
...
uint8_t SIM900HangUp(SIM900StateHandlePtr SIM900_State_Ptr){
        ...
        uint8_t answer=sendCommand1( SIM900_State_Ptr, AT_HANG, OK_RESPONSE);
    	return answer != 1 ? 0 : 1;
}
