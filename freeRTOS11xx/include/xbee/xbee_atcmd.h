...
*/
typedef PACKED_STRUCT xbee_cmd_response {
...
/// Returned by an xbee_cmd_callback_fn() if more responses are expected,
...
/**
	...
	requests.
*/
typedef PACKED_STRUCT xbee_cmd_request {
	/// Rolling identifier used to prevent use of stale request indices.
...
typedef union _xbee_header_at_request {
...
typedef union _xbee_frame_at_response {
	/// First byte of header determines type (local or remote).
	uint8_t								frame_type;
...
*/
typedef PACKED_STRUCT xbee_command_list_context_t {
...
*/
typedef PACKED_STRUCT xbee_atcmd_reg_t {
