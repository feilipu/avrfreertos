...
#elif defined __DC__
	#include "xbee_platform_rabbit.h"
#elif defined POSIX
	#include "xbee_platform_posix.h"
...
#define XBEE_TIMER_COMPARE( a, op, b) \
	( ((TickType_t)((TickType_t)(a) - (TickType_t)(b))) op 0 )
...
/**
	...
	@return	Value to assign to an unsigned 16-bit variable (TickType_t)
...
#define XBEE_SET_TIMEOUT_MS(delay)	\
	((TickType_t)xbee_millisecond_timer() + (delay))
...
/**
	...
	@param[in]	timer		16-bit (TickType_t) variable set by XBEE_SET_TIMEOUT_MS().
...
#define XBEE_CHECK_TIMEOUT_MS(timer) \
	((TickType_t)((TickType_t)xbee_millisecond_timer() - (timer)) >= 0)
...
/**
	...
	@return	Value to assign to an unsigned 16-bit variable (TickType_t) for use
	...
	@endcode
*/
#define XBEE_SET_TIMEOUT_SEC(delay) ((TickType_t)xbee_seconds_timer() + (delay))
...
/**
	...
	@param[in]	timer	TickType_t variable set by XBEE_SET_TIMEOUT_SEC().
...
#define XBEE_CHECK_TIMEOUT_SEC(timer) \
	((TickType_t)((TickType_t)xbee_seconds_timer() - (timer)) >= 0)

// include support for 64-bit integers
#include "jslong_glue.h"
