...
#else
	return (addr1 && addr2 && addr1->ll == addr2->ll );
#endif
}


/*** BeginHeader addr64_t_is_zero */
/*** EndHeader */
/** @brief
...
/*
	Do we need functions to return the high and low 32-bit halves of an addr64_t
	in host byte order?

	addr64_t_high32h and addr64_t_low32h?

	ntoh64 and hton64?

	I would prefer to ALWAYS store the bytes in an addr64_t structure in network
...
*/
int addr64_t_parse( addr64_t *address_be, const char FAR *str)
{
	uint_fast8_t i;
	uint8_t *b;
	int ret;
