...
uint16_t get_total(uint32_t from[])
{
	uint16_t total = 0;
...
void set_cell(uint32_t to[], int8_t x, int8_t y, uint8_t value)
{
	if(value)
		to[x] |= (uint32_t) 1 <<  y;
	else
...
void fill_cell(uint32_t to[], int8_t x, int8_t y)
{
	to[x] |= (uint32_t) 1 <<  y;
	return;
}
...
void clear_cell(uint32_t to[], int8_t x, int8_t y)
{
	to[x] &= ~( (uint32_t) 1 << y);
	return;
}
...
void fill_random_data(uint32_t to[])
{
...
uint8_t get_neighbours(uint32_t from[], int8_t x, int8_t y)
{
	uint8_t out = 0;
...
uint8_t get_difference(uint32_t a[],uint32_t b[])
{
	uint8_t diff=0;
...
void delayLong(uint16_t delayLocal)
{
	uint16_t delayvar = 0;
...
//
	...
	*/
			...
			if( generations++ > 511 || get_difference(current_generation,old_generation) < 8 || get_total(current_generation) < 6)
			{
