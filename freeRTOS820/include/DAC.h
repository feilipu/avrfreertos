...
#define Q_MAXIMUM 			(4.0f)
#define Q_LINEAR			(1/sqrt(2.0))
...
void setIIRFilter( filter_t *filter );
...
void setIIRFilterLPF( filter_t *filter ) // Low Pass Filter Setting
{
	...
	if ( !(filter->cutoff) )
		filter->cutoff = 0x8000; // half scale = filter->sample_rate>>2

	if ( !(filter->peak) )
		filter->peak = 0x2d41; // 65536/4 * 1/sqrt(2) = 11585  -> 1/sqrt(2)
...
//	DAC_out_P(ch_A_ptr, ch_B_ptr); // XXX Where ch_A_ptr and ch_B_ptr are pointers to values stored in PROGMEM
