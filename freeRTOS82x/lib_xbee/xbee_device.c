...
*/
_xbee_device_debug
int xbee_dev_init( xbee_dev_t *xbee, xbee_serial_t *usart,
											xbee_is_awake_fn is_awake, xbee_reset_fn reset)
{
	...
	xbee->serport = usart;			// copy the pointer to the port


	error = xbee_ser_open( xbee->serport, usart->baudRate );
	if (! error)
	{
	...
	if (! error)
	{
		error = xbee_ser_tx_flush( xbee->serport );
	}

	if (! error)
	{
		error = xbee_ser_rx_flush( xbee->serport );
	}

	xbee->flags &= ~XBEE_DEV_FLAG_USE_FLOWCONTROL; // xxx don't use flow control
...
#if !defined (_UNO_)
	xSerialxPrintf_P( &xSerial1Port, PSTR( "XBee on %s: HV=0x%x  VR=0x%" PRIx32 "  IEEE=%" PRIsFAR
		"  net=0x%04x\n\n"), xbee_ser_portname( xbee->serport ),
		xbee->hardware_version, xbee->firmware_version,
		addr64_t_format( addr, &(xbee->wpan_dev.address.ieee) ),
...
void _xbee_dispatch_table_dump( const xbee_dev_t *xbee)
{
	...
	for (i = 0; entry->frame_type != 0xFF; ++i, ++entry)
	{
		if (entry->frame_type)
		{
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%3d:\t0x%02x\t0x%02x\t0x%p\t%" PRIpFAR "\n"), i,
				entry->frame_type, entry->frame_id, entry->handler, entry->context);
		}
		else
		{
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%3d:\t[empty]\n"), i);
		}
	}
#endif
}
	...
	uint16_t headerlen, const void FAR *data, uint16_t datalen, uint16_t flags)
{
	...
	if (cts == -EINVAL)
	{
		...
		#ifdef XBEE_DEVICE_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: return -EINVAL (xbee = 0x%p, cts = %d)\n"),
	...
	{
		#ifdef XBEE_DEVICE_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: return -ENODATA (headerlen = %u, datalen = %u)\n"),
	...
	{
		#ifdef XBEE_DEVICE_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: return -EBUSY (cts = %s, free = %d, framesize = %d)\n"),
		...
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: frame type 0x%02x, id 0x%02x (%u-byte payload)\n"),
...
int _xbee_frame_load( xbee_dev_t *xbee)
{
	...
	{
		#ifdef XBEE_DEVICE_VERBOSE
		xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: return -EINVAL (xbee is %p)\n"), __FUNCTION__, xbee);
		#endif
		return -EINVAL;
	}
	...
	for (;;)
	{
	   switch (xbee->rx.state)
	   {
	      case XBEE_RX_STATE_WAITSTART:    // waiting for initial 0x7E
	         ...
	         #ifdef XBEE_DEVICE_VERBOSE
	         xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: got start-of-frame\n"), __FUNCTION__);
	         #endif
	   	...
	   	case XBEE_RX_STATE_LENGTH_MSB:
	   		...
	   		if (xbee_ser_read( serport, &ch, 1) == 0)
	   		{
				...
				if (ch == 0x7E)
				{
					...
					#ifdef XBEE_DEVICE_VERBOSE
					xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: ignoring duplicate start-of-frame (0x7E)\n"),
							__FUNCTION__);
					#endif
					break;
				}
	   	...
	   	case XBEE_RX_STATE_LENGTH_LSB:
	   		...
	   		// set LSB of frame length, make local copy for range check
				...
				if (length > XBEE_MAX_FRAME_LEN || length < 2)
				{
					...
					#ifdef XBEE_DEVICE_VERBOSE
					xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: read bad frame length (%u ! [2 .. %u])\n"),
	         ...
	         #ifdef XBEE_DEVICE_VERBOSE
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: got length %" PRIu16 "\n"), __FUNCTION__, length);
	         #endif
	      ...
	      case XBEE_RX_STATE_RXFRAME:      // receiving frame & trailing checksum
            ...
            xbee->rx.state = XBEE_RX_STATE_WAITSTART;
														...
														xbee->rx.bytes_in_frame + 1, 0xFF))
				{
					...
					#ifdef XBEE_DEVICE_VERBOSE
					xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: checksum failed\n"), __FUNCTION__);
				...
				else
				{
					...
					++dispatched;
					#ifdef XBEE_DEVICE_VERBOSE
					xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: dispatch frame #%d\n"), __FUNCTION__,
							dispatched);
					#endif
			...
			default:
				#ifdef XBEE_DEVICE_VERBOSE
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: invalid state %d\n"), __FUNCTION__,
						xbee->rx.state);
				#endif
				xbee->rx.state = XBEE_RX_STATE_WAITSTART;
				break;
...
void _xbee_dev_modem_status( wpan_dev_t *wpan, uint_fast8_t status)
{
	...
	switch (status)
	{
		...
		case XBEE_MODEM_STATUS_DISASSOC:
			...
			#ifdef XBEE_DEVICE_VERBOSE
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: marked network address as invalid (status=0x%02x)\n"),
					__FUNCTION__, status);
			#endif
			break;
	}
	...
	uint16_t length)
{
	...
	#ifdef XBEE_DEVICE_VERBOSE
	xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: dispatch frame type 0x%02x, id 0x%02x\n"),
	...
	for (entry = xbee_frame_handlers; entry->frame_type != 0xFF; ++entry)
	{
		if (entry->frame_type == frametype)
		{
			if (! entry->frame_id || entry->frame_id == frameid)
			{
	         ...
	         #ifdef XBEE_DEVICE_VERBOSE
				xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: calling frame handler @%p, w/context %" \
		...
		if (! dispatched)
		{
			xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: no handlers for frame type 0x%02x, id 0x%02x\n"),
				__FUNCTION__, frametype, frameid);
		}
	#endif
...
#if !defined (_UNO_)
	xSerialxPrintf_P( &xSerial1Port, PSTR( "%s: status: %s (0x%02x)\n"), __FUNCTION__,
